...
@property (nonatomic, strong) __block ESPSocketClient *socket;
...
- (ESPSocketClient *) open:(NSString *)remoteInetAddr
{
    ESPSocketClient *socket = nil;
    
    BOOL isConnected = NO;
    for (int retry = 0; !isConnected && retry < SO_CONNECT_RETRY; ++retry) {
        // connect to target device(root device)
        socket = [[ESPSocketClient alloc]init];
